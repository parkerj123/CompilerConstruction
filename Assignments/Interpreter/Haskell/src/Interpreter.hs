...
evalExp (ELtEq e1 e2)  = do
    ...
    val' <- evalExp e1
    val'' <- evalExp e2
    if val == VTrue || val' == val'' then return VTrue
    else return VFalse
    {-val <- evalExp e1
    val' <- evalExp e2
    val'' <- subValue val val'
    if applyFun ltValue e1 e2 == VTrue || val == val' then return VTrue
    --if val <= val' then return VTrue
    else return VFalse-}
evalExp (EGtEq e1 e2)  = do
    ...
    val' <- evalExp e2
    --if val' <= val then return VTrue
    if applyFun ltValue e2 e1 == VTrue || val == val' then return VTrue
    else return VFalse-}
...
evalExp (EAnd e1 e2) = do
    val <- evalExp e1
    val' <- evalExp e2
    if val == VTrue then return val'
    else return VFalse
evalExp (EOr e1 e2) = do
    val <- evalExp e1
    val' <- evalExp e2
    if val == VFalse then return val'
    else return VTrue
evalExp (EAss (EId i) e) = do
    val <- evalExp e
    updateContext i val
    return val 
evalExp (EAss i _) = fail $ "Expected " ++ printTree i ++ " to be an id."
{-evalExp (ETyped e _) = -}
